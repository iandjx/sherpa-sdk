...
// src/lib/MiMC.js
var MiMC_exports = {};
var circomlib, mimcsponge, snarkjs, bigInt, MimcSpongeHasher;
var init_MiMC = __esm({
  "src/lib/MiMC.js"() {
    init_cjs_shims();
    circomlib = require("circomlib");
    mimcsponge = circomlib.mimcsponge;
    snarkjs = require("snarkjs");
    bigInt = snarkjs.bigInt;
    MimcSpongeHasher = class {
...
var merkleTree_exports = {};
var jsStorage, hasherImpl, MerkleTree;
var init_merkleTree = __esm({
  "src/lib/merkleTree.js"() {
    init_cjs_shims();
    jsStorage = (init_Storage(), __toCommonJS(Storage_exports));
    hasherImpl = (init_MiMC(), __toCommonJS(MiMC_exports));
    MerkleTree = class {
...
var { bigInt: bigInt2 } = require("snarkjs");
var assert = require("assert");
var buildGroth16 = require("websnark/src/groth16");
var websnarkUtils = require("websnark/src/utils");
var merkleTree = (init_merkleTree(), __toCommonJS(merkleTree_exports));
var MERKLE_TREE_HEIGHT = 20;
var rbigint = (nbytes) => bigInt2.leBuff2int(randomBytes(nbytes));
function pedersenHash(data) {
  console.log("pedersen data");
  return circomlib2.babyJub.unpackPoint(circomlib2.pedersenHash.hash(data))[0];
}
function toHex(number, length = 32) {
  const str = number instanceof Buffer ? number.toString("hex") : bigInt2(number).toString(16);
...
async function generateProofSherpa(contract, deposit, recipient, events, circuit, provingKey, relayer = 0, fee = 0, refund = 0) {
  ...
  const input = {
    ...
    relayer: bigInt2(relayer),
    fee: bigInt2(fee),
    refund: bigInt2(refund),
